...
// //   try {
// //     const token = localStorage.getItem("token");
// //     const user = JSON.parse(localStorage.getItem("userInfo")); // from login
...
// import * as XLSX from "xlsx";
// import { saveAs } from "file-saver";
// import { FaFileExcel, FaFileCsv, FaTrash, FaEye } from "react-icons/fa";
// import jsPDF from "jspdf";
// import autoTable from "jspdf-autotable";
// const ManageOrders = () => {
//   const [orders, setOrders] = useState([]);
...
//     (o) =>
//       o._id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
//       (o.customerName || o.userId?.name)?.toLowerCase().includes(searchTerm.toLowerCase()) ||
//       (o.customerEmail || o.userId?.email)?.toLowerCase().includes(searchTerm.toLowerCase())
//   );
//   const currentItems = filtered.slice(indexOfFirst, indexOfLast);
//   const totalPages = Math.ceil(filtered.length / itemsPerPage);
//   // Select / Deselect
...
//       data.map((o) => ({
//         "Order ID": o._id,
//         "Customer Name": o.customerName || o.userId?.name,
//         "Customer Email": o.customerEmail || o.userId?.email,
//         "Products": o.products.map((p) => `${p.name} (${p.quantity})`).join(", "),
//         "Total Price": o.totalAmount,
//         "Status": o.status,
//         "Date": new Date(o.createdAt).toLocaleDateString(),
...
//       o.customerEmail || o.userId?.email,
//       o.products.map((p) => `${p.name} (${p.quantity})`).join(", "),
//       o.totalAmount,
//       o.status,
//       new Date(o.createdAt).toLocaleDateString(),
...
//               </td>
//               <td>{o._id}</td>
//               <td>{o.customerName || o.userId?.name}</td>
//               <td>{o.customerEmail || o.userId?.email}</td>
//               <td>{o.products.map((p) => `${p.name} (${p.quantity})`).join(", ")}</td>
//               <td>{o.totalAmount}</td>
//               <td>
//                 <Form.Select
...
//             <div>
//               <p><b>Order ID:</b> {currentOrder._id}</p>
//               <p><b>Customer:</b> {currentOrder.customerName || currentOrder.userId?.name} ({currentOrder.customerEmail || currentOrder.userId?.email})</p>
//               <p><b>Products:</b> {currentOrder.products.map(p => `${p.name} (${p.quantity})`).join(", ")}</p>
//               <p><b>Total Price:</b> {currentOrder.totalAmount}</p>
//               <p><b>Status:</b> {currentOrder.status}</p>
//               <p><b>Shipping:</b> {currentOrder.shippingAddress?.flat}, {currentOrder.shippingAddress?.area}, {currentOrder.shippingAddress?.city}, {currentOrder.shippingAddress?.state}, {currentOrder.shippingAddress?.country} - {currentOrder.shippingAddress?.pincode}</p>
...
const fetchOrders = async () => {
  try {
    ...
    // data is directly the array of orders (not inside data.orders)
...
const search = searchTerm.toLowerCase();
  const matchId = o._id?.toLowerCase().includes(search);
  const matchName = (o.customerName || o.userId?.name || "").toLowerCase().includes(search);
  const matchEmail = (o.customerEmail || o.userId?.email || "").toLowerCase().includes(search);
  const matchStatus = (o.status || "").toLowerCase().includes(search);
  const matchProducts = o.products?.some((p) =>
    p.name?.toLowerCase().includes(search)
  );
  const matchDate = new Date(o.createdAt).toLocaleDateString().toLowerCase().includes(search);
  return matchId || matchName || matchEmail || matchStatus || matchProducts || matchDate;
});
// Then paginate
const currentItems = filtered.slice(indexOfFirst, indexOfLast);
const totalPages = Math.ceil(filtered.length / itemsPerPage);
